...
`scripts/vla.sh` is the primary entry point for build/run/test against the
...
* `scripts/vla.sh run MODEL --remote {--cpu|--gpu} [--host BIND[:PORT]]` —
  host the cloud-side gRPC server.
* `scripts/vla.sh run MODEL --real --host HOST[:PORT]` — run the on-robot
  edge stack pointed at a remote.
* `scripts/vla.sh run MODEL --sim  --host HOST[:PORT]` — Gazebo + edge.
